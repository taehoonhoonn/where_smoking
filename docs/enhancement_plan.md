...
✅ **완료**: 좌표 노출 제거, 허위신고 관리자 기능, 신고 수 표시, 색상 구분, 마커 테두리 색상 수정, 관리자 "지도에서 보기" 기능, 흡연구역 필터 기능
❌ **미완료**: 역지오코딩, 영어 모드, 지도 검색 기능
...
- **구현 완료** (2025년 12월):
  1. ✅ **필터 상태 관리**: `Set<String> _activeFilters` 전역 상태로 3개 필터 추적 (공공데이터, 공식 시민제보, 비공식 시민제보)
  2. ✅ **인터랙티브 UI**: 기존 정적 범례 → 클릭 가능한 애니메이션 필터 버튼 전환
  3. ✅ **시각적 피드백**: 활성화 시 색상 테두리+체크 아이콘, 비활성화 시 회색 반투명 처리, 200ms 부드러운 애니메이션
  4. ✅ **실시간 마커 필터링**: `_shouldShowMarker()` 함수로 복잡한 조건 처리 (category + submitted_category 조합)
  5. ✅ **필터 지속성**: 지도 이동, 앱 복귀, 뷰포트 변경 시에도 필터 상태 유지 (`filteredOnly: true` 모든 렌더링에 적용)
  6. ✅ **데이터 매칭**: `'공공데이타'`, `'public_data'` 모두 공공데이터 필터에 매칭
- **구현된 UX 흐름**:
  - 지도 좌측 하단 필터 버튼 클릭 → 해당 카테고리 마커 실시간 토글 → 상태 메시지 "N개 표시됨 (총 M개 중)" 업데이트
- **검증 완료**:
  - ✅ 각 필터별 마커 정확한 표시/숨김 확인
  - ✅ 지도 이동, 앱 복귀 시 필터 상태 유지 확인
  - ✅ 복합 필터 조건 (시민제보 + 서브카테고리) 정상 작동
...
---
**현재 상태**: 7개 요구사항 완료, 3개 요구사항 남음 (구현 완료율 78%)

**다음 우선순위**:
1. **지도 검색 기능** (핵심 사용자 경험 완성, 카카오 Local API 연동)
2. **역지오코딩** (데이터 품질 향상, 시민제보 주소 자동 변환)
3. **영어 모드** (국제화, Flutter intl + l10n 설정)
